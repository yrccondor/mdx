...
<?php if(wp_get_attachment_url( get_post_thumbnail_id(get_the_ID()))){$Imagesurl=wp_get_attachment_url( get_post_thumbnail_id(get_the_ID()));}else{$Imagesurl="";}if($Imagesurl !== ""){?>
    <a href="<?php the_permalink();?>"><img src="data:image/gif;base64,R0lGODlhAgABAIAAALGxsQAAACH5BAAAAAAALAAAAAACAAEAAAICBAoAOw==" data-src="<?php echo $Imagesurl;?>" alt="<?php echo $Imagesurl;?>" title="<?php the_title();?>" class="LazyLoadList lazyload"></a>
<?php }else if(mdx_get_option("mdx_post_def_img")=="true" && $Imagesurl == ""){ ?>
    <a href="<?php the_permalink();?>"><img src="data:image/gif;base64,R0lGODlhAgABAIAAALGxsQAAACH5BAAAAAAALAAAAAACAAEAAAICBAoAOw==" data-src="<?php echo get_template_directory_uri().'/img/dpic.jpg'?>" alt="<?php echo get_template_directory_uri().'/img/dpic.jpg'?>" title="<?php the_title();?>" class="LazyLoadList lazyload"></a>
    <?php } ?>
