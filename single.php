    ...
    <?php if($post_style=="0"){if($full_image_url[0]!=""){$mdx_image_url=$full_image_url[0];}else{if(get_option("mdx_post_def_img")=="false"){$mdx_image_url=="";}else{$mdx_image_url=get_bloginfo("template_url")."/img/dpic.jpg";}}?>
      ...
      <div class="PostMain<?php if($mdx_image_url==""){?> mdx-pni-am0<?php }?>">
            ...
            <img class="PostMainImg0" alt="<?php the_title(); ?>" src="<?php echo $mdx_image_url;?>"><?php }else{?>
                ...
                <i class="mdui-icon material-icons">&#xe54e;</i> <?php if (get_the_tags()){the_tags('',' ','');}else{_e('没有标签','mdx');}?><span class="mdui-text-color-black-disabled timeInPost" itemprop="datePublished"><?php if(get_option('mdx_post_edit_time')==="post"){?><i class="mdui-icon material-icons info-icon">&#xe192;</i> <?php the_time('Y-m-d');}else{?><i class="mdui-icon material-icons info-icon">&#xe3c9;</i> <?php the_modified_time('Y-m-d');}?></span>
                <div class="mdui-divider"></div><?php mdx_breadcrumbs();?></div></div><?php endwhile;?><?php if(get_option('mdx_author_card')=="true"){include_once("includes/author_card.php");}if(get_option('mdx_you_may_like')=="true"){include_once("same_posts.php");}comments_template();?>
            </div>
<?php get_template_part('toggleposts')?>
      ...
      <?php }else if($post_style=="1"){if($full_image_url[0]!=""){$mdx_image_url = $full_image_url[0];}else{if(get_option("mdx_post_def_img")=="false"){$mdx_image_url=="";}else{$mdx_image_url=get_bloginfo("template_url")."/img/dpic.jpg";}}?>
        ...
        <div class="PostMain<?php if($mdx_image_url==""){?> mdx-pni-am<?php }?>">
            ...
            <?php if(get_option('mdx_author_card')=="true"){include_once("includes/author_card.php");}if(get_option('mdx_you_may_like')=="true"){include_once("same_posts.php");}comments_template();?>
<?php get_template_part('toggleposts')?>
        <div id="indic"></div>
