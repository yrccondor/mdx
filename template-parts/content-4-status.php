<div class="mdui-card staDivgaid mdui-center mdui-shadow-0">
<div class="mdui-card-media outofStaGaid">
    <div class="backGround"><i class="mdui-icon material-icons">&#xe0b7;</i> <?php _e('状态', 'mdx');?></div>
    <div class="staTime mdui-text-right"><i class="mdui-icon material-icons info-icon">&#xe192;</i> <?php the_time('Y-m-d');?></div>
    <article class="sayInStaGaid mdui-valign">
    <span><?php echo strip_shortcodes(strip_tags(apply_filters('the_content', $post->post_content)));?></span>
    </article>
</div>
</div>