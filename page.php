...
$mdx_index_img=mdx_get_option('mdx_index_img');$mdx_side_img=mdx_get_option('mdx_side_img');if($mdx_side_img==''){$mdx_side_img=$mdx_index_img;};$full_image_url = wp_get_attachment_image_src(get_post_thumbnail_id($post->ID),'full');?>
        ...
        <div class="<?php if($mdx_widget){?>mdx-tools-up-in <?php }?>PostMain PostMainPage">
            <div class="ArtMain mdui-center mdui-typo">
                ...
                <?php if((mdx_get_option('mdx_logged_in_ad')==="false" && !empty(mdx_get_option('mdx_ad'))) || ((mdx_get_option('mdx_logged_in_ad')==="true" && !is_user_logged_in()) && !empty(mdx_get_option('mdx_ad')))){
		            echo '<div class="mdx-ad-after-post">'.htmlspecialchars_decode(mdx_get_option('mdx_ad')).'</div>';
	            }?>
<?php endwhile;?>
            </div>
