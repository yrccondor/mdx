 ...
 * @param mixed $option_value 选项参数
 *
 * @return bool 必定为true
 */
function mdx_update_option( string $option_name, $option_value ): bool {
 ...
 * @return false|mixed
 */
function mdx_get_option( string $option_name ) {
 ...
 * 获取当前链接
 *
 * @param bool $is_post 是否为文章
 * @param int $post_id 文章ID
 *
 * @return string|WP_Error
 */
function mdx_get_now_url( bool $is_post = false, int $post_id = 0 ) {
	if ( $is_post ) {
 ...
 * @return array
 */
function mdx_menu_classes( array $classes, $item, $args ): array {
 ...
 * @param int $post_id 文章ID
 */
function get_post_views( int $post_id ) {
	$count_key = 'views';
 ...
 * 设置文章浏览量数据
 *
 * @param int|null $add 增加量
 */
function set_post_views( $add = 1 ) {
	global $post;
	if ( ! $post ) {
		return;
	}
	...
	if ( is_single() || is_page() ) {
		if ( $count == '' ) {
			...
			update_post_meta( $post_id, $count_key, $count + $add );
		}
	}
}
 ...
 * @return string
 */
function mdx_remove_wp_version(): string {
	return '';
}
 ...
 * @param array $args
 * @param int $depth 评论层级
 */
function mdx_comment_format( WP_Comment $comment, array $args, int $depth ) {
    ...
    <li class="mdui-list-item" id="li-comment-<?php comment_ID(); ?>">
        ...
        <div class="mdui-list-item-content outbu" id="comment-<?php comment_ID(); ?>">
            <div
                    class="mdui-list-item-title"><?php echo get_comment_author_link(); ?><?php if ( $comment->user_id === get_post( $comment->comment_post_ID )->post_author ) { // 好歹给多用户稍微适配一点吧？
            ...
            <span
                    class="mdx-reply-time"><?php echo human_time_diff( get_comment_time( 'U' ), current_time( 'timestamp' ) ) . __( '前', 'mdx' ); ?></span><?php comment_reply_link( array_merge( $args, array(
				'reply_text' => '<i class="mdui-icon material-icons">&#xe15e;</i> ' . __( '回复', 'mdx' ),
				'depth'      => $depth,
 ...
 * @return string
 */
function comment_add_at( string $comment_text, $comment ): string {
 ...
 * @return string
 */
function get_link_items(): string {
 ...
 * @return string
 */
function get_the_link_items( int $id = null ): string {
 ...
 * @return string
 */
function mdx_get_post_excerpt( WP_Post $post = null, int $excerpt_length = 150 ): string {
	if ( $post === null ) {
		$post = get_post();
	}
...
function mdx_process_image( $matches ) {
	...
	// Notice: !empty()比isset()快得多
	$mdx_sizes        = ! empty( $img['sizes'] ) ? ' sizes="' . $img['sizes']['value'] . '"' : '';
	$mdx_srcset       = ! empty( $img['srcset'] ) ? ' data-srcset="' . $img['srcset']['value'] . '"' : '';
	$mdx_img_style    = ! empty( $img['style'] ) ? ' style="' . $img['style']['value'] . '"' : '';
	$mdx_img_width    = ! empty( $img['width'] ) ? ' width="' . $img['width']['value'] . '"' : '';
	$mdx_figure_width = ! empty( $img['width'] ) ? 'max-width:' . $img['width']['value'] . 'px' : '';
	$mdx_img_height   = ! empty( $img['height'] ) ? ' height="' . $img['height']['value'] . '"' : '';
	$mdx_con_paading  = empty( $img['height'] ) || empty( $img['width'] ) ? 50 : $img['height']['value'] / $img['width']['value'] * 100;
	$mdx_img_class    = ! empty( $img['class'] ) ? $img['class']['value'] . ' ' : '';
	$mdx_img_alt      = ! empty( $img['alt'] ) ? ' alt="' . $img['alt']['value'] . '"' : '';
	$mdx_img_title    = $mdx_img_alt == '' || $mdx_img_alt == ' alt=""' ? ' title="' . get_the_title() . '"' : ' title="' . $img['alt']['value'] . '"';
	$mdx_img_alt      = $mdx_img_alt == '' || $mdx_img_alt == ' alt=""' ? ' alt="' . $img['src']['value'] . '"' : ' alt="' . $img['alt']['value'] . '"';
	if ( $mdx_img_class === "wp-smiley " ) {
		return $matches[0];
	}

	return mdx_get_option( "mdx_lazyload_fallback" ) == "true" ? sprintf( "<img %s%s class=\"%slazyload\" %s%s src=\"$placeholder_image\" data-src=\"%s\"%s%s%s>", $mdx_img_width, $mdx_img_height, $mdx_img_class, $mdx_img_style, $mdx_img_title, $img['src']['value'], $mdx_img_alt, $mdx_srcset, $mdx_sizes ) : sprintf( "<figure class=\"mdx-lazyload-container\" style=\"%s\"><div style=\"padding-top:%s%%\"></div><div class=\"mdx-img-loading-sp mdui-valign\"><div><div class=\"mdui-spinner\"></div></div></div><img%s%s class=\"%slazyload\"%s%s src=\"%s\" data-src=\"%s\"%s%s%s></figure>", $mdx_figure_width, strval( $mdx_con_paading ), $mdx_img_width, $mdx_img_height, $mdx_img_class, $mdx_img_style, $mdx_img_title, $placeholder_image, $img['src']['value'], $mdx_img_alt, $mdx_srcset, $mdx_sizes );
}
...
function mdx_process_avatar( $matches ) {
	...
	return /** @lang HTML */ "<img $mdx_img_width$mdx_img_height class=\"$mdx_img_class lazyload mdx-avatar-lazyload\" src=\"$placeholder_image\" data-src=\"{$img['src']['value']}\"$mdx_img_alt$mdx_srcset$mdx_sizes>";
}
...
function mdx_breadcrumbs() {
	...
	if ( ! is_home() && ! is_front_page() || is_paged() ) {
		...
		if ( is_category() ) {
			...
			if ( $thisCat->parent != 0 ) {
				$cat_code = get_category_parents($parentCat[0], true, ' ' . $delimiter . ' ' );
 ...
 * @return string
 */
function mdx_shortcode_hide( $atts, string $content = null ): string {
 ...
 * @return string
 */
function mdx_shortcode_warning( $atts, string $content = null ): string {
	$title = "";
	extract( shortcode_atts( array( "title" => __( '警告', 'mdx' ) ), $atts ) );

	return /** @lang HTML */ "<blockquote class=\"mdx-warning\"><p><i class=\"mdui-icon material-icons\">&#xe002;</i>$title<br><strong>" . do_shortcode( $content ) . "</strong></p></blockquote>";
}
 ...
 * @return string
 */
function mdx_shortcode_table( $atts, string $content = '' ): string {
	$header = "";
	...
	return "<div class=\"mdui-table-fluid$hoverable\"><table class=\"mdui-table\">$output2<tbody>$output</tbody></table></div>";
}
 ...
 * @return string
 */
function mdx_shortcode_progress( $atts, string $content = '0' ): string {
 ...
 * @return string
 */
function mdx_shortcode_post( $atts, string $content = '' ): string {
 ...
 * @return string
 */
function mdx_shortcode_ad( $atts, string $content = '' ): string {
 ...
 * @return string
 */
function mdx_shortcode_list( $atts, string $content = '' ): string {
 ...
 * @return string
 */
function mdx_shortcode_list_item( $atts, string $content = '' ): string {
	extract( shortcode_atts( array(
		'action'   => '',
		'blank'    => 'false',
		'avatar'   => '',
...
function mdx_colored_cloud_call_back( $matches ): string {
	$text  = $matches[1];
