...
<?php if(mdx_get_option('comment_registration') && !$user_ID):?>
<p class="mdx-comment-login-needed mdui-typo"><?php printf(__('你需要先 <a href="%s">登录</a> 才能发表评论.','mdx'), mdx_get_option('siteurl')."/wp-login.php?redirect_to=".urlencode(get_permalink()));?></p>
<?php else:
...
<?php $defaults = array(
		...
		'comment_field'=>'<div class="mdui-textfield mdui-textfield-floating-label'.$mdx_emj_cla.'"><i class="mdui-icon material-icons">textsms</i><label class="mdui-textfield-label">'.__('说点什么...','mdx').'</label><textarea class="mdui-textfield-input" name="comment" id="comment"></textarea></div>'.$mdx_emj_ele.$mdx_alu,
